...
CLAIM_PATTERNS = {
    "base_commit": re.compile(r"(?im)^\s*base\s+(?:commit|sha)\s*:\s*`?([0-9a-f]{40})`?\s*$"),
    "head_commit": re.compile(r"(?im)^\s*head\s+(?:commit|sha)\s*:\s*`?([0-9a-f]{40})`?\s*$"),
    "head_tree": re.compile(r"(?im)^\s*head\s+tree\s*:\s*`?([0-9a-f]{40})`?\s*$"),
}
...
def extract_claims(body: str) -> dict[str, str]:
    ...
    for name, pattern in CLAIM_PATTERNS.items():
        ...
        require(len(set(matches)) <= 1, f"PR body contains conflicting {name} claims")
        if matches:
...
def verify(
    ...
    require(SHA40.fullmatch(event_head) is not None, "event head is not a lowercase SHA-1")
    require(SHA40.fullmatch(event_base) is not None, "event base is not a lowercase SHA-1")
    ...
    require(local_head == event_head, "checked-out HEAD differs from pull-request event head")
    ...
    require(actual_head == event_head == local_head, "live PR head, event head and checkout differ")
    require(actual_base == event_base, "live PR base and event base differ")
    require(pull.get("state") == "open", "pull request is not open")
    require(not pull.get("draft"), "pull request is still draft")
    ...
    require(not missing_references,
    ...
    try:
        ...
        merge_ref_sha = merge_ref.get("object", {}).get("sha") if isinstance(merge_ref, dict) else None
    except SubjectError:
