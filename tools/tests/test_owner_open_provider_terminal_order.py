...
PROCESS_RETAINED = Path(
    "crates/trillionnium-owner-open-provider-jsonl/src/process_retained.rs"
)
...
class ProviderTerminalOrderingTests(unittest.TestCase):
    def test_process_exit_cannot_bypass_the_stdout_reader_queue(self) -> None:
        ...
        retained = PROCESS_RETAINED.read_text(encoding="utf-8")
        ...
        self.assertIn('#[path = "process_retained.rs"]', process)
        ...
        # the concrete process helper to remain in the retained lifecycle
        # module selected by the production facade.
        forced_cleanup = text.index("let cleanup = child.finish()")
        self.assertIn("finish_child(self.child_mut()?", retained)
