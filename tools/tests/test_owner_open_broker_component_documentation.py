...
""",
        ...
        for path, package in (("active", "active-package"), ("sealed", "sealed-package")):
            ...
            (directory / "src").mkdir()
            (directory / "src/lib.rs").write_text("// bounded fixture\n")
...
""",
    ...
    def test_catalog_cannot_map_to_nonexistent_source(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            self.write_fixture(root)
            (root / "active/src/lib.rs").unlink()
            with self.assertRaisesRegex(VERIFY.VerificationError, "source ownership path does not exist"):
                VERIFY.verify(root)

    def test_new_source_file_cannot_escape_module_inventory(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            self.write_fixture(root)
            (root / "active/src/unowned.rs").write_text("pub fn unowned() {}\n")
            with self.assertRaisesRegex(VERIFY.VerificationError, "source file needs exactly one module owner"):
                VERIFY.verify(root)

    def test_raw_html_blocks_cannot_supply_documentation_surfaces(self) -> None:
        # Strict accepted subset rejects block HTML rather than guessing where
        # the seven CommonMark HTML block types terminate.
        for opener, closer in (
            ("<div>", "</div>"), ("<div", "</div>"), ("<div/", "</div>"), ("<table>", "</table>"),
            ("<section class='x'>", "</section>"), ("<custom-element>", "</custom-element>"),
            ("<span>", "</span>"), ("<PRE>", "</PRE>"),
            ("<?processing", "?>"), ("<!DOCTYPE html>", ""),
            ("<![CDATA[", "]]>"), ("</div>", ""),
        ):
            for hidden in ("command", "link", "both"):
                with self.subTest(opener=opener, hidden=hidden), tempfile.TemporaryDirectory() as temporary:
                    root = Path(temporary)
                    self.write_fixture(root)
                    ...
                    block = []
                    for name, fragment in (("command", command), ("link", link)):
                        if hidden in (name, "both"):
                            prose = prose.replace(fragment, "")
                            block.append(fragment)
                    prose += "\n" + opener + "\n" + "\n".join(block) + "\n" + closer + "\n"
                    path.write_text(prose, encoding="utf-8")
                    with self.assertRaisesRegex(VERIFY.VerificationError, "unsupported raw HTML block"):
                        VERIFY.verify(root)

    def test_inline_html_and_fenced_html_examples_preserve_real_navigation(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            self.write_fixture(root)
            path = root / "active/README.md"
            path.write_text(path.read_text() + "\nA <span>bounded</span> component.\n\n```html\n<div>\nexample\n</div>\n```\n")
            VERIFY.verify(root)

    def test_readme_link_cannot_replace_catalog_source_ownership(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            self.write_fixture(root)
            path = root / "docs/machine/module-catalog.v1.json"
            catalog = json.loads(path.read_text())
            catalog["modules"][0]["paths"] = ["sealed/src/lib.rs"]
            path.write_text(json.dumps(catalog))
            with self.assertRaisesRegex(VERIFY.VerificationError, "has no module contract mapping"):
                VERIFY.verify(root)

    def add_support_component(self, root: Path, *, declared: bool) -> None:
        cargo = root / "Cargo.toml"
        cargo.write_text(cargo.read_text().replace('["active", "sealed"]', '["active", "sealed", "support"]').replace('default-members = ["active"]', 'default-members = ["active", "support"]'))
        support = root / "support"
        support.mkdir()
        (support / "Cargo.toml").write_text('[package]\nname="support-package"\nversion="0.1.0"\nedition="2024"\n')
        (support / "README.md").write_text('# support-package\n\n[MOD-SUPPORT](../docs/modules/MOD-SUPPORT.md)\n\n```sh\ncargo test --locked -p support-package --all-targets\n```\n\n' + 'Bounded source component. ' * 20)
        (root / "docs/modules/MOD-SUPPORT.md").write_text('# Support module\n')
        path = root / "docs/machine/module-catalog.v1.json"
        catalog = json.loads(path.read_text())
        catalog["default_source_closure"].append("support")
        catalog["modules"][0]["dependencies"] = ["MOD-SUPPORT"] if declared else []
        catalog["modules"].append({"id": "MOD-SUPPORT", "paths": ["support"], "dependencies": []})
        path.write_text(json.dumps(catalog))

    def test_undeclared_local_dependency_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            self.write_fixture(root)
            self.add_support_component(root, declared=False)
            manifest = root / "active/Cargo.toml"
            manifest.write_text(manifest.read_text() + '\n[dependencies]\nsupport-package={path="../support"}\n')
            with self.assertRaisesRegex(VERIFY.VerificationError, "undeclared module dependency"):
                VERIFY.verify(root)

    def test_renamed_inherited_optional_target_dependency_is_projected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            self.write_fixture(root)
            self.add_support_component(root, declared=True)
            cargo = root / "Cargo.toml"
            cargo.write_text(cargo.read_text() + '\n[workspace.dependencies]\nsupport-alias={package="support-package",path="support"}\n')
            manifest = root / "active/Cargo.toml"
            manifest.write_text(manifest.read_text() + '\n[target.\'cfg(unix)\'.dependencies]\nsupport-alias={workspace=true,optional=true}\n')
            VERIFY.verify(root)
            path = root / "docs/machine/module-catalog.v1.json"
            catalog = json.loads(path.read_text())
            catalog["modules"][0]["dependencies"] = []
            path.write_text(json.dumps(catalog))
            with self.assertRaisesRegex(VERIFY.VerificationError, "undeclared module dependency"):
                VERIFY.verify(root)

    def test_development_dependency_cannot_import_sealed_component(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            self.write_fixture(root)
            manifest = root / "active/Cargo.toml"
            manifest.write_text(manifest.read_text() + '\n[dev-dependencies]\nsealed-package={path="../sealed"}\n')
            with self.assertRaisesRegex(VERIFY.VerificationError, "enters sealed non-product"):
                VERIFY.verify(root)
