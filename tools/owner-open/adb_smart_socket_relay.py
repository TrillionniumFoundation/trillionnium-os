#!/usr/bin/env python3
"""Canonical bounded byte-transparent ADB smart-socket relay.

The relay transports bytes only. It never parses an ADB frame or service name,
never injects serial/host/port arguments, and never retries an uncertain effect.
"""
    ...
    async def accept(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        ...
        try:
            ...
            transfers = asyncio.create_task(
                asyncio.gather(
                    pump(reader, upstream_writer, state, "client_to_upstream"),
                    pump(upstream_reader, writer, state, "upstream_to_client"),
                ),
    ...
    async def serve(self) -> int:
        ...
        if self.connections:
            ...
            _done, pending = await asyncio.wait(
                list(self.connections), timeout=self.limits.shutdown_grace
            )
            for task in pending:
                task.cancel()
