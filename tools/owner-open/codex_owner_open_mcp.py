...
correlation scope for the MCP server lifetime, and never classifies commands,
requires approval, rewrites arguments, or automatically retries uncertainty.
"""
...
class Server:
    ...
    def dispatch(self, message: dict[str, Any]) -> None:
        ...
        if method == "initialize":
            ...
            key, token = self.id_key(request_id), threading.Event()
            with self.pending_lock:
...
def host_argv(args: argparse.Namespace) -> tuple[list[str], Scope]:
    ...
    if not 0 < args.startup_timeout <= 120 or not 0 < args.request_timeout <= 600:
        raise InvalidArguments("startup/request timeout is outside the finite bound")
    scope = Scope(
        require_id(args.session_id or generated("mcp-session"), "session_id"),
