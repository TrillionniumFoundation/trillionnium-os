...
def normalized_repo_path(root: Path, value: Any, label: str) -> tuple[str, Path]:
    ...
    require(raw == pure.as_posix() and "\\" not in raw, f"{label} must be POSIX-normalized")
...
def parse_workspace(root: Path) -> tuple[list[str], list[str]]:
    ...
    for index, member in enumerate(members):
        ...
        require(manifest.is_file() and not manifest.is_symlink(), f"workspace member Cargo.toml missing: {member}")
    return members, defaults


def verify_lifecycle(root: Path, members: list[str], defaults: list[str], catalog: dict[str, Any]) -> None:
    lifecycle = load_json(root / "governance/component-lifecycle.v1.json")
    exact_keys(lifecycle, LIFECYCLE_KEYS, "component lifecycle")
    require(lifecycle["schema"] == "org.trillionnium.component-lifecycle.v1", "unsupported lifecycle schema")
    require(lifecycle["authority"] == "docs/machine/module-catalog.v1.json", "lifecycle authority drifted")
    text(lifecycle["program_revision"], "lifecycle.program_revision")
    text(lifecycle["rule"], "lifecycle.rule")

    catalog_defaults = strings(catalog.get("default_source_closure"), "catalog.default_source_closure")
    require(defaults == catalog_defaults, "Cargo default-members drift from catalog default_source_closure")
    ...
    for index, entry in enumerate(entries):
        require(isinstance(entry, dict), f"non_product_members[{index}] must be an object")
        exact_keys(entry, LIFECYCLE_ENTRY_KEYS, f"non_product_members[{index}]")
        path, _ = normalized_repo_path(root, entry["path"], f"non_product_members[{index}].path")
        excluded.append(path)
        require(path in member_set, f"lifecycle entry is not a Cargo workspace member: {path}")
        require(path not in default_set, f"active default member is classified non-product: {path}")
        classification = text(entry["classification"], f"{path}.classification")
        require(classification.startswith("sealed_"), f"non-product classification is not sealed: {path}")
        reason = text(entry["reason"], f"{path}.reason")
        require(len(reason) >= 32, f"non-product reason is too short: {path}")
        replacements = strings(entry["replacement"], f"{path}.replacement", empty=True)
        for replacement in replacements:
            replacement_path, absolute = normalized_repo_path(root, replacement, f"{path}.replacement")
            require(absolute.exists(), f"replacement path does not exist: {replacement_path}")

    expected_excluded = [member for member in members if member not in default_set]
    require(excluded == expected_excluded, "non-product lifecycle entries must exactly follow workspace member order")

    for member in defaults:
        readme = root / member / "README.md"
        require(readme.is_file() and not readme.is_symlink(), f"active component README missing: {member}")
        require(len(readme.read_bytes()) >= 256, f"active component README is truncated: {member}")


def verify_modules(root: Path, catalog: dict[str, Any]) -> None:
    modules = catalog.get("modules")
    require(isinstance(modules, list) and bool(modules), "catalog.modules must be a non-empty array")
    index = load_json(root / "docs/machine/module-document-index.v1.json")
    indexed = index.get("modules")
    require(isinstance(indexed, list), "module document index modules must be an array")
    index_map = {
        text(entry.get("id"), "module index id"): text(entry.get("doc_path"), "module index path")
        for entry in indexed
        if isinstance(entry, dict)
    }
    require(len(index_map) == len(indexed), "module document index contains malformed or duplicate entries")

    ids: list[str] = []
    api_schemas: set[str] = set()
    state_schemas: set[str] = set()
    for position, module in enumerate(modules):
        require(isinstance(module, dict), f"catalog.modules[{position}] must be an object")
        module_id = text(module.get("id"), f"catalog.modules[{position}].id")
        ids.append(module_id)
        expected_doc = f"docs/modules/{module_id}.md"
        require(index_map.get(module_id) == expected_doc, f"module document mapping drifted: {module_id}")
        document = root / expected_doc
        require(document.is_file() and not document.is_symlink(), f"module document missing: {module_id}")
        ...
        for path_index, source in enumerate(paths):
            source_text, source_path = normalized_repo_path(root, source, f"{module_id}.paths[{path_index}]")
            require(source_path.exists(), f"module source path missing: {source_text}")
            require(f"`{source_text}`" in prose, f"module prose omits source path {source_text}")

        api = module.get("api_contract")
        state = module.get("state_contract")
        require(isinstance(api, dict) and isinstance(state, dict), f"{module_id} API/state contract missing")
        api_schema = text(api.get("schema"), f"{module_id}.api.schema")
        state_schema = text(state.get("schema"), f"{module_id}.state.schema")
        require(SCHEMA_RE.fullmatch(api_schema) is not None, f"malformed API schema: {api_schema}")
        require(SCHEMA_RE.fullmatch(state_schema) is not None, f"malformed state schema: {state_schema}")
        require(api_schema not in api_schemas, f"duplicate API schema: {api_schema}")
        require(state_schema not in state_schemas, f"duplicate state schema: {state_schema}")
        api_schemas.add(api_schema)
        state_schemas.add(state_schema)
        require(f"`{api_schema}`" in prose, f"module document omits API schema: {module_id}")
        require(f"`{state_schema}`" in prose, f"module document omits state schema: {module_id}")
        for field in ("inputs", "outputs", "errors"):
            values = strings(api.get(field), f"{module_id}.api.{field}")
            for value in values:
                require(TYPE_RE.fullmatch(value) is not None, f"placeholder API type in {module_id}: {value}")
                require(f"`{value}`" in prose, f"module document omits API type {value}")

    require(len(ids) == len(set(ids)) == 16, "G1 catalog must contain exactly 16 unique modules")
    require(set(ids) == set(index_map), "catalog and module document index sets differ")


def verify_workflows(root: Path) -> None:
    for relative in sorted(REQUIRED_WORKFLOWS):
        path = root / relative
        require(path.is_file() and not path.is_symlink(), f"required permanent workflow missing: {relative}")
        body = path.read_text(encoding="utf-8")
        require("permissions:" in body, f"workflow lacks explicit permissions: {relative}")
        require("contents: write" not in body, f"workflow may write repository contents: {relative}")
        require("pull-requests: write" not in body, f"workflow may mutate pull requests: {relative}")
        require("persist-credentials: false" in body or "actions/checkout" not in body,
                f"workflow checkout persists credentials: {relative}")
        for match in re.finditer(r"uses:\s*([^\s]+)", body):
            action = match.group(1)
            require("@" in action and not action.endswith(("@main", "@master", "@v1", "@v2", "@v3", "@v4")),
                    f"workflow action is not commit-pinned: {relative}: {action}")

    target = (root / ".github/workflows/owner-open-r5-target-evidence-capture.yml").read_text(encoding="utf-8")
    require("workflow_dispatch:" in target, "target evidence workflow is not manually dispatched")
    require("pull_request:" not in target and "push:" not in target,
            "target evidence workflow must not run automatically")
    for level in range(2, 7):
        require(f"owner-open-r5-l{level}" in target, f"target workflow omits L{level} runner/environment")
    require("/opt/owner-open-r5/harnesses/" in target, "target workflow omits fixed harness root")
    require("/etc/owner-open-r5/attestations/" in target, "target workflow omits fixed attestation root")
    require("synthetic" in target and "false" in target, "target workflow omits synthetic-evidence rejection")

    governance = (root / ".github/workflows/owner-open-r5-governance-readiness.yml").read_text(encoding="utf-8")
    require("OBSERVE_ONLY" in governance, "governance workflow must declare observe-only posture")
    require("merge_pull_request" not in governance and "enable_auto_merge" not in governance,
            "governance workflow contains a merge mutation")

    compatibility = (root / ".github/workflows/owner-open-r5-tool-loop.yml").read_text(encoding="utf-8")
    require("verify_g1_repository_closure.py" in compatibility,
            "compatibility workflow does not verify the real repository closure")
    require("ALIAS_ONLY_NO_EVIDENCE_AUTHORITY" in compatibility,
