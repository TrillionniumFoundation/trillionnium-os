...
use std::sync::mpsc::{Receiver as OuterReceiver, SyncSender as OuterSender, sync_channel as outer_channel};
use std::thread;

use trillionnium_owner_open_job_runtime::{JobManager, JobRuntimeConfig};
...
pub(crate) fn run() -> Result<(), String> {
    ...
    let (outer_sender, outer_receiver) = outer_channel(JOB_HOST_QUEUE_DEPTH);
    ...
    thread::Builder::new()
        .name("owner-open-v6-turn-core".to_string())
        .spawn(move || {
...
impl JobHostOptions {
    fn parse(args: Vec<OsString>) -> Result<Self, String> {
        ...
        while index < args.len() {
            ...
            match option {
                ...
                _ => {
                    forwarded.push(args[index.saturating_sub(1)].clone());
                }
            }
        }
        Ok(Self {
    ...
    fn usage() -> String {
        format!(
            "{}\n\nLong-running job options:\n  --job-store /absolute/path/jobs.jsonl\n  --require-job-journal\n\nThe job-aware v6 core multiplexes job.start/inspect/attach/detach/write/resize/close_stdin/kill with the existing v4 turn core. The selected v5 transport remains responsible for bounded persisted delivery flow control.",
            Options::usage()
        )
    }
}
