...
use trillionnium_owner_open_turn_loop::{
    ProviderEvent, TurnCancellation, TurnEvent, TurnEventKind,
...
const HOST_IMPLEMENTATION: &str = "trillionnium-owner-open-r5-streaming-host-source";
    ...
    loop {
        ...
        match frame.kind.as_str() {
            ...
            FRAME_TURN_START => {
                ...
                if let Err(error) = write_frame(&mut writer, &accepted, limits.max_frame_bytes) {
                ...
                let run_result = {
                    let mut sink = |event: &TurnEvent| -> Result<(), String> {
                        if let Some(frame) = map_turn_event(&mut output, &context, event) {
                            let frame =
                                persist_for_delivery(persistence, &scope, &digest, frame);
                            if client_attached {
                                if let Err(error) =
                ...
                let (status, summary, error) = match run_result {
                    Ok(run) => (
                        run.terminal.status.as_str().to_string(),
                ...
                let terminal = output.frame(
                    ...
                    json!({
                        ...
                        "event_log_error": persistence.error(),
                        "streaming_events": true,
                        "client_delivery_status": if client_attached { "attached" } else { "detached" },
                        "client_delivery_error": delivery_error
                    }),
                    Some(&context),
                    EventCorrelation::default(),
                );
                ...
                if client_attached {
                    if write_frame(&mut writer, &terminal, limits.max_frame_bytes).is_err() {
                        return Ok(());
                    }
                } else {
                    return Ok(());
            ...
            FRAME_TURN_CANCEL => {
                write_frame(
                    ...
                    &output.frame(
                        ...
                        json!({
                            "code": "no_active_turn",
                            "message": "the current stdio carrier cannot service turn.cancel while the provider thread owns the connection",
