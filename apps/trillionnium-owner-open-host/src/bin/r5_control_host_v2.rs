...
fn handle_turn_cancel<W: Write>(
    ...
    if request.session_id != active.context.session_id
        ...
        || request
            .profile_id
            .as_deref()
            .is_some_and(|value| value != active.context.profile_id.as_str())
        || request
            .task_id
            .as_deref()
            .is_some_and(|value| value != active.context.task_id.as_str())
        || request
            .turn_stream_id
            .as_deref()
            .is_some_and(|value| value != active.context.turn_stream_id.as_str())
    {
        deliver_host_error(
            writer,
            output,
...
fn handle_tool_cancel<W: Write>(
    ...
    let payload_call_id = frame.payload.get("call_id").and_then(Value::as_str);
...
fn validate_control_context(frame: &RunTurnFrame, context: &TurnContext) -> Result<(), String> {
    for (label, envelope, expected) in [
        ...
        if let Some(value) = frame.payload.get(label) {
            let value = value
                .as_str()
                .ok_or_else(|| format!("tool.cancel payload {label} must be a string"))?;
            if value != expected {
                return Err(format!(
