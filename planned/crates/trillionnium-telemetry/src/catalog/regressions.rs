...
fn operation_dimension_rejects_sixty_fifth_distinct_value() {
    ...
    for index in 0..64 {
        ingestor
            .ingest(event(index, index + 1, &format!("op-{index}")))
            .unwrap();
    }
...
fn idle_watermark_expires_samples_without_reinserting_duplicate() {
    ...
    ingestor
        .advance_watermark("broker.accept.duration_ms", "broker-1", 1, 1_000_000_002)
        .unwrap();
...
fn watermark_regression_unknown_domain_and_capacity_failure_do_not_mutate() {
    ...
    assert!(
        ingestor
            .advance_watermark("broker.accept.duration_ms", "broker-1", 1, 9)
            .is_err()
    );
    assert!(
        ingestor
            .advance_watermark("broker.accept.duration_ms", "other", 1, 20)
            .is_err()
    );
    assert!(
        ingestor
            .advance_watermark("broker.accept.duration_ms", "broker-1", 1, 70_000_000_000)
            .is_err()
    );
    assert_eq!(before, snapshot(&ingestor));
}
...
fn late_observation_and_clock_regression_are_explicit_atomic_errors() {
    ...
    assert!(
        ingestor
            .ingest(event(0, 11, "accept"))
            .unwrap_err()
            .to_string()
            .contains("LATE_OBSERVATION")
    );
    assert!(
        ingestor
            .ingest(event(2, 9, "accept"))
            .unwrap_err()
            .to_string()
    ...
    assert_eq!(before, snapshot(&ingestor));
    ingestor
        .advance_watermark("broker.accept.duration_ms", "broker-1", 1, 20)
        .unwrap();
...
fn observed_gaps_and_cardinality_are_not_reset_by_time_eviction() {
    ...
    ingestor.ingest(event(0, 1, "accept")).unwrap();
    ingestor
        .advance_watermark("broker.accept.duration_ms", "broker-1", 1, 2_000_000_000)
        .unwrap();
