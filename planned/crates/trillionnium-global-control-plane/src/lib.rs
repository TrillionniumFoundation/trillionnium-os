...
impl AdmissionReservation {
    ...
    pub fn validate(&self, now_ms: u64, new_effect: bool) -> Result<AdmissionDecision> {
        ...
        state.validate_admission(&self.lease, now_ms, self.slots, new_effect)
    }
...
impl Controller {
    ...
    pub fn shadow_decision_and_record(&self, generated_at_ms: u64) -> Result<ShadowDecision> {
        ...
        let state = self
            ...
            .lock()
            .map_err(|_| ControlError::Invalid("controller state lock poisoned".into()))?;
        let decision = state.shadow_decision(generated_at_ms)?;
        let mut audit = self
            .decision_audit
            .inner
            .lock()
            .map_err(|_| ControlError::Invalid("decision audit lock poisoned".into()))?;
        DecisionAuditLog::record_locked(&mut audit, &decision, generated_at_ms)?;
        Ok(decision)
    }
    ...
    pub fn rollback_shadow_decision(
        ...
        let mut audit = self
            ...
            .lock()
            .map_err(|_| ControlError::Invalid("decision audit lock poisoned".into()))?;
        DecisionAuditLog::rollback_locked(&mut audit, target_digest, expected_epoch, recorded_at_ms)
    }
    ...
    /// no recommendation mutates a lease or authorizes an effect.
    pub fn shadow_decision(&self, generated_at_ms: u64) -> Result<ShadowDecision> {
        let state = self
            .inner
            .lock()
            .map_err(|_| ControlError::Invalid("controller state lock poisoned".into()))?;
        state.shadow_decision(generated_at_ms)
    }
    ...
    pub fn admit(
        ...
        let state = self
            ...
            .lock()
            .map_err(|_| ControlError::Invalid("controller state lock poisoned".into()))?;
        state.validate_admission(lease, now_ms, requested_slots, new_effect)
    }
...
impl ControllerState {
    ...
    fn observe(&mut self, observation: ModuleObservation) -> Result<()> {
    ...
    /// Return a deterministic recommendation.  It is a read-only projection;
    /// no recommendation mutates a lease or authorizes an effect.
    fn shadow_decision(&self, generated_at_ms: u64) -> Result<ShadowDecision> {
        let observations = self.observations.values().collect::<Vec<_>>();
        ...
        for observation in observations {
            ...
            // either case would make two replicas disagree about the digest;
            // failing closed keeps the decision deterministic and forces the
            // caller to obtain a fresh lease/observation set.
            ...
            let recommended = if observation.health_score < 0.5 || observation.unknown_rate > 0.0 {
                1
            } else if observation.queue_depth > lease.resource_budget.max_queue / 2 {
                lease.maximum_concurrency
            } else {
                lease
                    .maximum_concurrency
            ...
            recommendations.push(BudgetRecommendation {
                ...
                reason_code: if observation.health_score < 0.5 {
                    "health_guard"
                } else if observation.unknown_rate > 0.0 {
                    "unknown_guard"
                } else if observation.queue_depth > lease.resource_budget.max_queue / 2 {
                    "queue_pressure"
                } else {
...
mod tests {
    ...
    fn lease_expiry_and_fencing_are_fail_closed() {
        ...
        let lease = controller
            .issue_lease("broker", "b-1", "p0", 10, None)
            .unwrap();
    ...
    fn shadow_decisions_are_reproducible_and_non_semantic() {
        ...
        let lease = first.issue_lease("broker", "b-1", "p0", 10, None).unwrap();
        ...
        let lease2 = second.issue_lease("broker", "b-1", "p0", 10, None).unwrap();
    ...
    fn outage_allows_existing_lease_but_stops_new_effects_and_leases() {
        let controller = Controller::new(ControlMode::Shadow, 1, config()).unwrap();
        let lease = controller
            .issue_lease("jobs", "j-1", "p0", 10, None)
            .unwrap();
    ...
    fn controller_indexes_are_bounded_and_expired_leases_are_reclaimed() {
        ...
        let second = controller
            .issue_lease("jobs", "j-2", "p0", 110, None)
            .unwrap();
    ...
    fn observation_index_rejects_new_entries_after_its_bound() {
        ...
        let first = controller
            ...
            .unwrap();
        let second = controller
            .issue_lease("jobs", "j-2", "p0", 10, None)
            .unwrap();
    ...
    fn controller_and_reservation_are_send_sync_and_clones_share_state() {
        ...
        let lease = clone.issue_lease("jobs", "j-1", "p0", 10, None).unwrap();
    ...
    fn stale_reservation_drop_cannot_release_a_new_epoch_token() {
        ...
        let old_lease = controller
            .issue_lease("jobs", "j-old", "p0", 10, None)
            .unwrap();
        ...
        let new_lease = controller
            .issue_lease("jobs", "j-new", "p0", 20, None)
            .unwrap();
    ...
    fn epoch_rotation_fences_shared_registry_and_audit_handles() {
        ...
        let new_lease = controller
            .issue_lease("jobs", "j-2", "p0", 30, None)
            .unwrap();
