# MOD-TURN-ENGINE — same-turn engine

This document is the detailed source-development, integration and qualification contract for `MOD-TURN-ENGINE`. The machine authority remains `docs/machine/module-catalog.v1.json`; this document explains how engineers must implement and operate that contract without widening its evidence ceiling.

## 1. Identity and maturity

- Module ID: `MOD-TURN-ENGINE`
- Module version: `1.0.0`
- Name: **same-turn engine**
- Plane: `semantic-adapter`
- Primary owner: `team-turn-engine`
- Backup owner: `team-provider`
- Maturity: `SOURCE_EVENT_DRIVEN_CANCEL_PENDING_EVIDENCE`
...
The engine opens a bounded turn stream, forwards provider events, registers exact call identities, waits on event-driven completion or targeted cancellation, and emits exactly one terminal turn record. Per-turn retention is finite.
...
Cancellation is scoped to the exact call or turn identity. The engine does not allocate global resources, reinterpret provider intent, poll a broad process namespace, or convert a timeout into a false success.
...
Direct dependencies: `MOD-PROTOCOL`, `MOD-TOOL-RUNTIME`.
...
- API schema: `org.trillionnium.mod_turn_engine.api.v1`
- Catalog input labels: `turn_request_v1`
- Catalog output labels: `turn_event_v1`
- Catalog error labels: `turn_error_v1`
...
- Implementation source: `crates/trillionnium-owner-open-turn-loop/src/lib.rs` — `TurnRunner`
...
`TurnRunner` coordinates a `SameTurnProvider` through `ProviderHost` and publishes `TurnEvent` values to `TurnEventSink`. `TurnCancellation` is scoped to this turn. Terminal retention and cancellation races must be tested separately from external effect completion.

## 6. State model and ownership

- State schema: `org.trillionnium.mod_turn_engine.state.v1`
...
A turn is bound to one engine epoch. Upgrade drains active turns or marks them interrupted; it does not transfer live cancellation handles across epochs.
...
Expose active turns, retained event items and bytes, targeted cancellation latency, duplicate terminal attempts, provider wait time and terminal classification.
...
- Verification source: `crates/trillionnium-owner-open-turn-loop/tests/streaming_and_cancel.rs`
...
For a nonterminating turn, inspect exact call identities and event retention, trigger only the targeted inhibit/cancel path, preserve terminal races, and avoid process-wide kill unless the separately authorized emergency procedure requires it.
...
Open machine gaps: `GAP-CONC-TURN-CANCEL-001`.
