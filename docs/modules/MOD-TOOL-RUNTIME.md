# MOD-TOOL-RUNTIME — direct tool and process runtime

This document is the detailed source-development, integration and qualification contract for `MOD-TOOL-RUNTIME`. The machine authority remains `docs/machine/module-catalog.v1.json`; this document explains how engineers must implement and operate that contract without widening its evidence ceiling.

## 1. Identity and maturity

- Module ID: `MOD-TOOL-RUNTIME`
- Module version: `1.0.0`
- Name: **direct tool and process runtime**
- Plane: `execution`
- Primary owner: `team-runtime`
...
- exact shell and ADB process mechanics.

Operationally, the required flow is:

The runtime receives an exact provider-selected operation, reserves capacity, establishes output readers before unblocking a writer, spawns the process with bounded identity and environment, streams bounded observations and converges descendants on cancellation or shutdown.
...
The runtime executes exact bytes and arguments. It does not maintain a semantic command allowlist, rewrite shell text, append a device serial, change working intent or retry an operation whose effect may already have occurred.
...
- API schema: `org.trillionnium.mod_tool_runtime.api.v1`
- Catalog input labels: `tool_call_v1`
- Catalog output labels: `tool_result_v1`
- Catalog error labels: `tool_error_v1`
...
- Implementation source: `crates/trillionnium-owner-open-tool-bridge/src/lib.rs` — `BoundToolCall`
...
`BoundToolCall` couples request identity to `DirectToolRequest`; `DirectToolBridge` returns `DispatchResult` while the shared owner-open runtime controls processes. Keep dispatch ownership, stdout/stderr delivery, process exit and descendant cleanup as separate observations.
...
Live process handles are not portable across runtime epochs. Upgrade fences admission, records interruption and reconciles descendants before resuming.
...
Record call ID, executable identity, argv digest, environment policy version, process-group or cgroup identity, stdout/stderr byte counts, truncation, exit and cleanup classification.
...
- Verification source: `crates/trillionnium-owner-open-tool-bridge/tests/failure_closure.rs`
...
On leaked descendants, inhibit the affected runtime shard, preserve process/cgroup observations, execute bounded cleanup, and keep the call unknown until descendant absence is independently observed.
