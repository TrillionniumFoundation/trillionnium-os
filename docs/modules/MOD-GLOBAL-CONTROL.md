# MOD-GLOBAL-CONTROL — global mechanical control plane

This document is the detailed source-development, integration and qualification contract for `MOD-GLOBAL-CONTROL`. The machine authority remains `docs/machine/module-catalog.v1.json`; this document explains how engineers must implement and operate that contract without widening its evidence ceiling.

## 1. Identity and maturity

- Module ID: `MOD-GLOBAL-CONTROL`
- Module version: `1.0.0`
- Name: **global mechanical control plane**
- Plane: `control`
- Primary owner: `team-global-control`
- Backup owner: `team-architecture`
- Maturity: `PLANNED_SHADOW_SOURCE_COMPLETE_PENDING_CI`
...
The controller may allocate finite mechanical resources but cannot select goals, tools, commands, targets, retries or compensations. Objective inputs are bounded, versioned and auditable.
...
Direct dependencies: `MOD-PROTOCOL`, `MOD-TELEMETRY`.
...
- API schema: `org.trillionnium.mod_global_control.api.v1`
- Catalog input labels: `control_request_v1`
- Catalog output labels: `control_decision_v1`
- Catalog error labels: `control_error_v1`
...
- Implementation source: `planned/crates/trillionnium-global-control-plane/src/lib.rs` — `Controller`
...
- State schema: `org.trillionnium.mod_global_control.state.v1`
- State authority: **authoritative**
- Partition key: `module_id`
- State owned: `control epoch; module leases; placement plan`
...
Loss of controller authority expires leases and stops new globally governed admission while bounded local work follows its declared outage policy. Stale epochs cannot write authoritative control state.
...
Controller state starts in observe-only and shadow modes. Epoch transitions fence stale decisions; active control is not enabled by a source-only migration.
...
- Verification source: `planned/crates/trillionnium-global-control-plane/src/lib.rs`
...
On unstable proposals, remain in observe or shadow, preserve inputs and deterministic outputs, disable promotion to active and investigate objective or telemetry drift without changing semantic behavior.
