...
- Catalog input labels: `execution_request_v1`
- Catalog output labels: `execution_event_v1`
- Catalog error labels: `execution_error_v1`
...
`JobStartWire` and `JobControlWire` are decoded into job requests without semantic rewriting. `decode_job_wait` supplies bounded read-only observation, not cancellation. `runtime_job_frame` binds runtime observations back to exact job scope and transport identity.
