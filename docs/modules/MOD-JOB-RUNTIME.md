# MOD-JOB-RUNTIME — durable long-running jobs

This document is the detailed source-development, integration and qualification contract for `MOD-JOB-RUNTIME`. The machine authority remains `docs/machine/module-catalog.v1.json`; this document explains how engineers must implement and operate that contract without widening its evidence ceiling.

## 1. Identity and maturity

- Module ID: `MOD-JOB-RUNTIME`
- Module version: `1.0.0`
- Name: **durable long-running jobs**
- Plane: `execution`
- Primary owner: `team-job-runtime`
- Backup owner: `team-runtime`
- Maturity: `SOURCE_SHARDED_START_PENDING_EVIDENCE`
...
The job runtime is not a scheduler of semantic goals. It acts only on explicit job operations, preserves exact identifiers and cannot infer restart intent from a crash or cleanup outcome.
...
Direct dependencies: `MOD-PROTOCOL`, `MOD-EVENT-STORE`.
...
- API schema: `org.trillionnium.mod_job_runtime.api.v1`
- Catalog input labels: `job_operation_v2`
- Catalog output labels: `job_event_v2`
- Catalog error labels: `job_error_v1`
...
- Implementation source: `crates/trillionnium-owner-open-job-runtime/src/manager.rs` — `JobManager`
...
`JobManager` admits a job under an `AdmissionPermit`, journals acceptance, and publishes process identity. `JobJournal` distinguishes accepted operations from terminal observations; its durable records must be committed before replay can return an existing terminal without a new process.

## 6. State model and ownership

- State schema: `org.trillionnium.mod_job_runtime.state.v1`
- State authority: **authoritative**
- Partition key: `job_id`
- State owned: `job registry; job operation journal; live job handles`
...
Expose admission rejection, reservation count, shard contention, spawn stages, durable transition latency, descendant state, stream cursor and unknown outcome counts.
...
- Verification source: `crates/trillionnium-owner-open-job-runtime/tests/runtime.rs`
...
### GAP-CONC-JOB-START-HOTLOCK-001 — exit L2

Spawn and durability slow paths stay outside the global running-map lock.

Exit evidence must demonstrate:
- capacity token is reserved atomically.
- per-key transition is linearizable.
- unrelated jobs start concurrently.
- fault cuts release reservations.
