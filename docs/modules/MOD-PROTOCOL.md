# MOD-PROTOCOL — protocol and identity

This document is the detailed source-development, integration and qualification contract for `MOD-PROTOCOL`. The machine authority remains `docs/machine/module-catalog.v1.json`; this document explains how engineers must implement and operate that contract without widening its evidence ceiling.

## 1. Identity and maturity

- Module ID: `MOD-PROTOCOL`
- Module version: `1.0.0`
- Name: **protocol and identity**
- Plane: `shared-contract`
- Primary owner: `team-protocol`
- Backup owner: `team-architecture`
- Maturity: `L1_SOURCE`
...
Callers construct a versioned envelope, canonicalize identity fields, validate limits and unknown-field policy, and pass only the validated value to the next mechanical module. Invalid identity, duplicate members, oversized fields, or unsupported versions fail before an effect is admitted.
...
Identity material is data, not authority. Parsing never grants permission, chooses a command, retries an effect, or turns an unauthenticated peer into a trusted principal.
...
- API schema: `org.trillionnium.mod_protocol.api.v1`
...
- Implementation source: `crates/trillionnium-owner-open-types/src/lib.rs` — `RunTurnFrame`
...
- State authority: **non-authoritative**
- Partition key: `protocol-specific`
- State owned: none
- Durability class: `none`
...
- Ordering key: `protocol-specific`
...
No persisted state migration is required for the v1 stateless protocol module.
...
Emit counters for decode outcome, version, size rejection and canonicalization failure. Do not emit command bodies, credentials, provider prompts or raw secrets.
...
- Verification source: `crates/trillionnium-owner-open-types/src/lib.rs`
...
On incompatible protocol traffic, capture the exact version and bounded structural error, verify the producer version, and stop admission. Never add an implicit compatibility fallback in an incident.
...
Open machine gaps: none.

This means the catalog currently assigns no unresolved implementation or evidence gap directly to this module. It does not elevate the repository, installation, device or release as a whole; transitive dependencies and the global evidence chain retain their own claim ceilings.
