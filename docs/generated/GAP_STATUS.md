...
| `GAP-CONC-BROKER-MUX-001` | `P0` | `concurrency` | `SOURCE_CLOSED_PENDING_EVIDENCE` | `L2` | MOD-BROKER, MOD-TRANSPORT | Replace the single active effect request with bounded multi-inflight multiplexing. |
| `GAP-CONC-EVENT-STORE-001` | `P0` | `concurrency` | `SOURCE_CLOSED_PENDING_EVIDENCE` | `L2` | MOD-EVENT-STORE | Replace the single-file single-lock write hotspot with indexed segmented durability. |
| `GAP-CONC-JOB-START-HOTLOCK-001` | `P0` | `concurrency` | `SOURCE_CLOSED_PENDING_EVIDENCE` | `L2` | MOD-JOB-RUNTIME | Remove the global running-map lock from spawn and durability slow paths. |
